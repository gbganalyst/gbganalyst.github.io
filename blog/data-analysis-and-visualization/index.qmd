...
```{r}
#| include: false
install.load::install_load(c("reticulate",
...
clean_data <- clean_data %>% 
  mutate(autism = fct_relevel(autism, "YES"),
...
```{r}
fh_yes <- clean_data %>% filter(autismFH == "yes") %>% pull(score)
fh_no <- clean_data %>% filter(autismFH == "no") %>% pull(score)
